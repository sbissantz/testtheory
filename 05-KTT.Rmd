...
## The burden of complex measurment
...
- Erklärung: True Score $\sim$ Observed Score $\sim$ Measurement Error.

## Lösungsvorschlag: Das True Score Approach 

\begin{equation}
\Large \tau = X + \epsilon \Leftrightarrow X = \tau - \epsilon  
\end{equation}

- $X$: observed score (known)
- $\tau$: true score (unknown)
- $\epsilon$: error component (unknown)
...
\begin{block}{Grundannahme}
$E(\epsilon) = 0$ (die Fehler sind unsystematisch)
\end{block}
...
M <- 10 # Verändere mich!
# Messung duzende (M) Male wiederholen
...
## Die True Score Logik: ein Gedankenexperiment
...
hist(reps, main = NULL, xlab="Messwiederholungen", ylab="Häufigkeiten: Erwartungswerte")
mtext("Intraindividuelle Merkmalsverteilung")
```
...
```{r echo=TRUE}
set.seed(123)
# Ein tatsächlicher Beobachtungswert (Zufallszug)
(X_i <- sample(X, 1))
...
# Anzahl der (sonst imaginären) Testwiederholungen
M <- 5000 # (5000): Verändere mich!
...
```{r, out.width="80%", results='hide', fig.align='center'}
hist(T, main = NULL, xlab = "True Scores")
mtext("Verteilung der wahren Intelligenzwerte in der Population")
```
...
```{r, out.width="80%", results='hide', fig.align='center'}
hist(rapply(X, mean), main = NULL, xlab="Erwartungswert der M Intelligenztests")
mtext("Veteilung der Erwartungswerte in der Population")
```
...
old.par <- par(no.readonly = TRUE)
par(mfrow=c(2,2))
lapply(rnd_smpl, hist, main=NULL, xlab="Testwerte der Person")
par(old.par)
```
