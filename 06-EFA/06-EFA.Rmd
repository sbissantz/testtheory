...
Zusatz: Vergleichen Sie die Ergebnisse der PAF mit denen der PCA. Bestehen
Unterschiede zwischen den Ergebnissen? Haben Sie eine Vermutung wie diese
zustande kommen?

<!-- PAFA ist vereinfacht gesagt eine iterative PCA der reduzierten -->
<!-- Korrelationsmatrix mit dem Ziel die gemeinsame Varianz aufzuklären. (siehe v.a.: -->
<!-- Selbststudium 3) -->

## Übungsaufgabe 5: Kritische Reflexion
...
Zusatz: Vergleichen Sie die Ergebnisse der MLF mit denen der PAF. Bestehen
Unterschiede zwischen den Ergebnissen? Haben Sie eine Vermutung warum diese
zustande kommen?
...
<!-- Hinweis: Die Entscheidung zur orthogonalen Rotation ist in den -->
<!-- Sozialwissenschaften deutlich entscheidender als die Wahl eines bestimmten -->
<!-- Rotationskriterium! -->

## Orthogonale Rotation: Varimax

...maximiere die Varianz der quadrierten Ladungen
...
simuliert! Entspricht das soeben erhaltene Ergebnis dem "richtigen" Ergebnis?
Wenn ja, warum? Falls nein, warum nicht?
...
2. Lösung: Scree-Test 
3. Lösung. Horns Parllel Analyse
...
## Kaiser Kriterium im Scree Plot

\begin{block}{Kaiserkriterium}
...Extrahiere so lange Hauptkomponenten bis diese weniger Varianz aufklären als
eine einzelne Variable aus dem Datensatz. 
\end{block}

```{r echo=TRUE, message=FALSE, warning=FALSE, results='hide'}
# Barfuß 
ev <- eigen(R)$values
plot(ev, main="Scree Plot", xlab="Eigenwerte", 
...
# Händisch
ev <- eigen(R)$values
plot(ev, main="Scree Plot", xlab="Komponentenzahl", 
...
```{r}
# psych::fa.parallel(X, plot = TRUE)
# psych::fa.parallel(Depnum, fa = "pc", cor = "poly",fm = "ml")
...
Für  wir die Faktoren in Anbetracht eines unterbestimmten Modells (2 Unbekannte)
...
## Zusatzmaterial: PCA versus EFA

<!-- EFA concentrates on explaining the off-diagonal elements of Σ, whereas PCA -->
<!-- concentrates on the diagonal elements of Σ. Thus, EFA tries to explain -->
<!-- covariance in the data, and PCA tries to explain variance in the data (although -->
<!-- it is not totally blind to the covariances) (Mair, 2018, S. 186) -->
...
\begin{block}{EFA = PCA}
 EFA und PCA entsprechen sich, genau dann wenn man beim berechnen der Kennwerte
 die Korrelationsmatrix, nicht die reduzierte Korrelationsmatrix verwendet wird
 und man als Extraktionsmethode eine Eigenwertzerlegung der Korrelationsmatrix
 bzw. eine Singulärwertzerlegung der Datenmatrix durchführt.
\end{alertblock}

## Zusatzmaterial: die Relevanz von Iterationen

...bei der Anpassungsgüte des Modells in der PAFA
...
## Selbststudium 1: händische PCA -- Eigenwertzerlegung
...
## Selbststudium 2: händische EFA -- Singulärwertzerlegung
...
## Selbststudium 3: reduzierte Korrelationsmatrix
...
## Selbststudium 3: Hintergrundwissen
...
original 1s on the diagonal with the the value of $1 - u^2$ where $U^2 =
diag(R−FF')$. 
