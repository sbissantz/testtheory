...
Hausarbeiten zu ergattern. Schauen Sie sich den Fragebogen einmal an. Wenn Sie
möchten, beantworten Sie ihn kurz. Damit unterstüzten Sie das Open Science
Projekt. Wenn Sie fertig sind, stellen wir uns der Large Data Set Challenge!
\end{example}
...
(LDC) lautet: Erkennen Sie eine Struktur in den Daten? D.h., wenn ja weiter;
welche Items könnten man Ihrer Meinung nach zu Itemgruppen zusammenfassen?
\end{example}
...
Wichtig: "meaningful compression" (Faktoren) vs. "full compression" (Komponenten)
...
par(old.par)
```
Anmerkung: Ist diese Folie leer, gibt es in \texttt{psych} immer noch keinen Fix
für den Output von Strukturmodellen. Im R-Skript sollte aber alles einwandfrei
funktionieren.
...
\begin{example}
Versuchen Sie es nun selbst! Führen sie eine PCA durch. Interpretieren Sie die
...
\begin{example}
 Versuchen Sie es nun selbst! FÜhren Sie eine Maximum Likelihood Faktorenanalyse
 (MLF) durch. Rotieren Sie anschließend die Ergebnisse mit der Varimax und
 Oblimin Rotation. Wie verändert die Rotation ihre Interpretation der
 Ergebnisse? Erleichtert sie sie? Vergleichen SIe die Lösung dazu zum einen mit
 der unrotierten Lösung und vergleichen Sie zum anderen die
 Ergebnisse zwischend den Rotationsmethoden.
\end{example}
...
Sie die Ergebnisse? Sind ihre Matritzen zur Analyse geeignet?
\end{example}

Zusatz: Können Sie eine Situation konstruieren, in denen die Tests die
analysetauglichkeit verneinen?
...
Anmerkung: Führen Sie die Ergebnisse solange durch, bis sich die Verändeurngen
in der Hauptdiagonale stabilisieren
